    ...
    <div className="w-full overflow-hidden py-6">
      <div className="flex animate-marquee whitespace-nowrap">
        {[...badges, ...badges].map((badge, index) => (
          <div
            key={`${badge.id}-${index}`}
            className=" inline-flex items-center gap-5 mx-4 px-2 py-2 text-gray-800 bg-bgTertiary bg-opacity-25 backdrop-blur-sm rounded-full shadow-md cursor-pointer 
            transform transition-all duration-800 hover:scale-110 hover:shadow-lg min-w-[29vw] h-20"
          ...
          >
            ...
            <div>
              <p className="text-md w-[22vw] whitespace-normal text-white/70">
                "{badge.text}"
              </p>
              <p className="text-sm text-white/50 float-end">~ {badge.name}</p>
            </div>
          </div>
        ))}
      </div>
